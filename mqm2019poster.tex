...
\begin{columns}[t]
	...
	\begin{column}{\colwidth}
		...
		\begin{block}{Introduction}
			...
			We have located these solutions using \textbf{\color{red} SCF metadynamics} and investigated their \textbf{\color{red} symmetry properties} in a model octahedral $d^2$ system, \ce{[VF6]^{3-}} (\cref{fig:d2_allnoci}).
	  	\end{block}
	...
	\begin{column}{\colwidth}
		...
		\begin{block}{Non-Orthogonal Configuration Interaction (NOCI)}
			For a symmetry-broken set $\mathrm{S}$, solving the \textbf{\color{Blue} generalised eigenvalue equation}
				\begin{equation*}
		...
		\begin{block}{UHF vs. NOCI: Jahn--Teller Distortion}
			Consider the $T_{1g} \otimes e_g$ Jahn--Teller distortion in octaheral \ce{[VF6]^{3-}}. \Cref{fig:JT} shows that:
				\begin{itemize}
					...
					\item the $\prescript{3}{}{T}_{1g}[\mathrm{A}_1 \oplus \mathrm{A}'_1]$ \textbf{\color{Blue} NOCI wavefunctions} do \textbf{\color{Red} show the expected stabilisation} and give the \textbf{\color{Red} correct degeneracy splitting}.
				\end{itemize}
		...
		\begin{block}{Solution Topology: Euclidean Realisation of State Distances}
			...
			By realising the distance matrices between symmetry-broken HF solutions as \textbf{\color{Blue} polytopes} showing their \textbf{\color{Blue} arrangements in Euclidean space} (\cref{fig:AAdashpolytopes}), we hope to gain insight into the nature of their symmetry breaking.
